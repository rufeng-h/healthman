...
class PtClass(MySQL):
    ...
    def _prepare(self):
        ...
        self.mappings = {code: name for code, name in self.cursor.fetchall()}
        self.cursor.close()
        self.conn.close()

    def to_excel(self):
        self.class_df['college'] = self.class_df['college_id'].apply(lambda x: self.mappings.get(str(x)))
        ...
        self.class_df = self.class_df[['code', 'name', 'college_name', '录入年份', '年级']].copy()
