...
class ShortRunning:
    ...
    def _gen_next_year(self, gender, ls):
        print("生成下一年")
        cs = []
        chgs = [i for i in range(0, 400)]
        chgs.extend(i for i in range(-400, 0))
        for cur in ls:
            s = copy.copy(cur)
            year = cur['year'] + 1
    ...
    def _data(self, gender):
        arg1 = (2300, 5200) if gender == 'M' else (1800, 3500)
        arg2 = (3100, 4300) if gender == 'M' else (2000, 3000)
...
Path(f'./data/vital{gender}.json').write_text(json.dumps(data, ensure_ascii=False, indent=4), encoding='utf-8')
